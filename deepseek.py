    ...
    def __init__(self, args: ModelArgs):
        ...
        def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
            ...
            if attn_impl == "naive":
                ...
                # not naive implementation but instead deepseeks implementation
                wkv_b = self.wkv_b.weight if self.wkv_b.scale is None else weight_dequant(self.wkv_b.weight, self.wkv_b.scale, block_size)
                wkv_b = wkv_b.view(self.n_local_heads, -1, self.kv_lora_rank)
                q_nope = torch.einsum("bshd,hdc->bshc", q_nope, wkv_b[:, :self.qk_nope_head_dim]) # transform merge
                self.kv_cache[:bsz, start_pos:end_pos] = self.kv_norm(kv) # save normalized key value into cache
                self.pe_cache[:bsz, start_pos:end_pos] = k_pe.squeeze(2) # same with positional
            ...
            # deepseek MoE (mixture of experts) // GATE
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, inter_dim: int):
        ...
        """
        super().__init__()
        self.w1 = ColumnParallelLinear(dim, inter_dim) # if we have 1 GPU it will act as a linear layer
